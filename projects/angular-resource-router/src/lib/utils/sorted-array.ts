 ...
 * @internal
 */
const locationFor = <T>(element: T, array: T[], comparer: Comparer<T>, start: number, end: number): number => {
  if (array.length === 0) {
    return -1;
  }
 ...
 * @internal
 */
const insertLocation = <T>(element: T, array: T[], comparer: Comparer<T>): number => {
