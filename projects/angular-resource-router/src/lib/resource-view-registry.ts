 ...
 * @internal
 */
const validateType = (config: ViewDef) => {
 ...
 * @internal
 */
const validateStatus = (config: ViewDef) => {
  if (config.status) {
 ...
 * @internal
 */
const validateStatusExpression = (config: ViewDef, status: string) => {
 ...
 * @internal
 */
const newValidationError = (config: any, text: string): Error =>
  new Error(`Invalid view configuration, ${text}:\n${JSON.stringify(config)}`);

// Utils
/**
 * @internal
 */
export const normalizeStatusExpression = (pattern: string): string => {
  // Handle * wildcard
  if (pattern.endsWith('*')) {
    if (pattern.length > 3) {
    ...
    return (`${pattern.replace(/\*/g, '')}???`).substring(0, 3);
  }
 ...
 * @internal
 */
const qualityComparator = (a: { quality: number }, b: { quality: number }) => {
 ...
 * @internal
 */
const toArray = <T>(value: T | T[]): Array<T> =>
